...
pdf_dir_name = "pdf"
text_dir_name = "text"
xlsx_path = "data2/result_all_leak.xlsx"

# settings
context_expand = 8
user_agent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/105.0.0.0 Safari/537.36"
base_url_fmt = "http://www.safehoo.com/Case/Case/Blow/List_{}.shtml"
max_page_index = 57
...
accident_keyword_regex = get_keyword_regex(accident_word_list)
crash_keyword_regex = get_keyword_regex(crash_word_list)
logger = logging.getLogger("mainlogger")
logger.setLevel(logging.DEBUG)
set_logger("webCrawler4.log", logger)
...
# continue from last
resume = False
resume_page_cnt = 32 # start from 1, reverse
...
accident_matcher = tools.WordMatcher(accident_keyword_regex)
crash_matcher = tools.WordMatcher(crash_keyword_regex)
for i in range(strat_cnt, max_page_index+1):
    ...
    with xw.App(visible=False,add_book=False) as app:
        ...
        # sheet2
        if len(wb.sheets) < 2:
            sht2 = wb.sheets.add("crash")
            xl_header = ["标题","id","url","本地链接","","","关键词上下文"]
            sht2.range('A1').value = xl_header
            wb.save(xlsx_path)
        else:
            sht2 = wb.sheets[1]
        ...
        for article_idx in range(article_one_page):
            ...
            try:
                ...
                if text is not None:
                    accident_match_res = accident_matcher.get_all_context(text)
                    if accident_match_res is not None and len(accident_match_res)>0:
                        res = content_matcher.get_all_context(text)
                        if res is not None and len(res) > 0:
                            # match crash:
                            crash_res = crash_matcher.get_all_context(text)
                            accident_match_res.extend(res) # inner modification on accident_match_res
                            logger.info(f"title {title}, pass")
                            if crash_res is not None and len(crash_res) > 0:
                                # if match
                                accident_match_res.extend(crash_res)
                                sht2.range(f"A{cur_row}").value = title
                                sht2.range(f"C{cur_row}").add_hyperlink(page_url)
                                sht2.range(f"H{cur_row}").value = accident_match_res
                                wb.save(xlsx_path)
                            else:
                                sht.range(f"A{cur_row}").value = title
                                sht.range(f"C{cur_row}").add_hyperlink(page_url)
                                sht.range(f"H{cur_row}").value = accident_match_res
                                wb.save(xlsx_path)
                        else:
                            logger.info(f"title {title}, not pass")
                driver.close()
            except Exception as e:
